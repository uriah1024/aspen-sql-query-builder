...
from Pando_Dashboard_ML import topic_extraction,create_dict_list_of_topics
import time
...
# Set up the app
app = dash.Dash()
#data = pd.read_csv('/python/reddit_test.csv')
#data['title'] = data['title'].str.title()
#reddit_post_df = pd.DataFrame(data)
data = pd.read_csv('/python/reddit_test.csv')
data['body'].replace(r'^\s*$', np.nan, regex=True)
data.sort_values(by=['comms_num'],ascending=False)
#print(data)
final_reddit_topic_df = topic_extraction(data)
...
def timeline_top_post_filtered(top_topic_filtered_df, selected_dropdown_value):
    # Make a timeline
    top_topic_filtered_df['timestamp'] = (top_topic_filtered_df['timestamp']/100.0)
    print(top_topic_filtered_df['timestamp'])
